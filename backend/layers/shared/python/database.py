import os
from sqlalchemy import create_engine, Column, String, Integer, UUID, TIMESTAMP, ForeignKey, Boolean, Numeric
...
class LeagueTeamSchoolAssignment(Base):
    ...
    league_id = Column(PG_UUID(as_uuid=True), primary_key=True)
...
class LeagueDraft(Base):
    ...
    current_league_id = Column(PG_UUID(as_uuid=True))
    current_user_id = Column(PG_UUID(as_uuid=True))
